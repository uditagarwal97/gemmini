...
void setFI(reg_t rs1, reg_t rs2) {
	...
	int fault_model = (rs1 >> 41) & 0x00000007;
	int fault_data = (rs1 >> 44);
	dprintf("tile_row:%d, tile_col:%d, pe_row:%d, pe_col:%d, do_fi:%d, fault_model:%d, fault_data:%d\n",
...
reg_t gemmini_t::CUSTOMFN(XCUSTOM_ACC)(rocc_insn_t insn, reg_t xs1, reg_t xs2) {
  ...
  } else if (insn.funct == fault_injection_cmd) {
	  setFI(xs1, xs2);
  }
